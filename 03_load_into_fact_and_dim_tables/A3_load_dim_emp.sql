...
-- Step 0: Ensure job_id table exists
DO $$
BEGIN
  BEGIN
...
-- Step 1: Insert new departments from staging to dim_department
...
-- Step 2: Add row_hash to staging if not exists
DO $$ BEGIN
...
-- Step 3: Map department_id from dim_department into staging_employee
DO $$ BEGIN
...
-- Step 4: Expire existing dim_employee rows that have changed
...
-- Step 5: Insert new or changed rows (SCD2 logic with CASE for valid_from)
...
-- Step 6: Audit log for dim_employee insert
...
-- Step 7: Load snapshot into fact_employee
...
-- Step 8: Audit log for fact_employee snapshot
WITH inserted_rows AS (
  SELECT COUNT(*) AS count
